...
# Main settings
WORKING_ION = "Li"  # Primary working ion for initial data retrieval
NEW_WORKING_IONS = ["Na"]  # List of new working ions to substitute
    ...
    if not SKIP_CALCULATIONS:
        ...
        for i, calc_type in enumerate(CALC_TYPES):
            if CALC_IDX_INITS[i] == CALC_IDX_FINALS[i]:
                ...
                add_energy_forces_to_df(df_main, WORKING_ION, calc_type, FILE_DIRPATH, CALC_IDX_INITS[i],
                                        CALC_IDX_FINALS[i])
    else:
